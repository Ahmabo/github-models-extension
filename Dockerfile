oFROM node:20.16.0-alpine AS builder
WORKDIR /usr/src/app
COPY package*.json ./
RUN npm ci
COPY . .
...
䡍Y .codesnack-ide/PK
䡍Yî B ± ± .codesnack-ide/metadata.json{"version":"1.0","name":"(  ت ن م ك ا ؟)","isWebProject":false,"config":{"version":"1.0","runConfigurations":[{"name":"Default","isDefault":true,"command":"yarn start"}]}}PK
䡍Yn~~ŉ ‰
index.htmlPK
䡍Yç탸^ ^ ± index.jsPK
䡍Yý‘  5
